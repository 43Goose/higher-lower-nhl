...
export default function Home() {
  const btnStyle = 'flex justify-center items-center m-4 w-32 h-14 bg-cyan-600 rounded-full hover:scale-110 active:scale-95 active:bg-cyan-700';
  let acceptedCookies = false; // try using the check function instead of this first
  ...
  function cookiesDisplay() {
    return(
      <div>
        <div>
          <p>Accept cookies?</p>
          <p>This website uses cookies ONLY to keep your high score.</p>
          <button className={btnStyle}>ACCEPT</button>
          <button className={btnStyle}>DECLINE</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24">
      ...
      <div className="flex flex-col items-center w-1/2 max-w-500px z-10">
        ...
        <div className={`${fugaz.className} flex flex-row w-full items-center justify-center`}>
          <Link href={{ pathname: '/classic', query: {cookies: true}}} className={btnStyle}>CLASSIC</Link>
          <Link href={{ pathname: '/goals', query: {cookies: true}}} className={btnStyle}>GOALS</Link>
          <Link href={{ pathname: '/assists', query: {cookies: true}}} className={btnStyle}>ASSISTS</Link>
        </div>
      </div>
    </main>
  );
